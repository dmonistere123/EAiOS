...
describe('Knowledge page', () => {
  ...
  it('agent-scoped source requires selecting at least one agent', async () => {
    const user = userEvent.setup();
    render(
      <MemoryRouter>
        <Knowledge />
    ...
    await user.click(screen.getByRole('button', { name: 'Add source' }));
    await user.click(screen.getByRole('tab', { name: 'Add URL' }));
    await user.type(screen.getByPlaceholderText('https://…'), 'https://allygnment.com/quill-only');

    const scopeSelect = screen.getByLabelText('Scope');
    await user.selectOptions(scopeSelect, 'agent');

    // Agent picker appears
    expect(await screen.findByText('Allowed agents')).toBeInTheDocument();
    const allyCheckbox = screen.getByRole('checkbox', { name: /Ally/ });
    await user.click(allyCheckbox);

    const spy = vi.spyOn(knowledge, 'addUrl');
    await user.click(screen.getByRole('button', { name: /Add & index/i }));
    await waitFor(() => expect(spy).toHaveBeenCalledTimes(1), { timeout: 4000 });
    const [_calledUrl, calledMeta] = spy.mock.calls[0];
    expect((calledMeta as { scope: string }).scope).toBe('agent');
    expect((calledMeta as { allowedAgentIds: string[] }).allowedAgentIds).toEqual(['ally']);
  });
});
